...
Troubleshooting
---------------
- “No executable found” during container build:
  - Ensure your target installs a binary to a `bin/` path; entrypoint discovery now covers common `usr/local/bin` and a fallback scan for ELF executables.
- “Neither podman nor docker found”:
  - Install one of them or remove `CONTAINER` from `GENERATORS`.
