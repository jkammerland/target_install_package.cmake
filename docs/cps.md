...
The [Common Package Specification](https://cps-org.github.io/cps/) is a standard package metadata format for installed libraries and tools. Its purpose is cross-build-system consumption: tools can read the same declarative package data instead of interpreting build-system-specific package scripts. Package managers and distribution tooling can ship or generate CPS metadata as ecosystem support develops. A `.cps` file describes package components, versions, configurations, and link requirements in data form instead of CMake script code. The specification is developed at [`cps-org/cps`](https://github.com/cps-org/cps), and CMake 4.3 can generate CPS metadata with [`install(PACKAGE_INFO)`](https://cmake.org/cmake/help/latest/command/install.html#package-info).
...
- This wrapper also rejects CMake 4.4 `SOURCES` file sets for CPS exports. CMake 4.4.2 installs the source files but `install(PACKAGE_INFO)` omits their file-set metadata from the generated `.cps` file, so a CPS-discovered consumer cannot compile them. The regular `Config.cmake` export remains authoritative for source packages. Re-evaluate this limitation when upgrading the tested CMake version.
...
- `CPS_APPENDIX` packages should use their own `EXPORT_NAME`; do not mix root package targets and appendix targets in one export. Appendices cannot set root CPS metadata options such as `CPS_PROJECT`, `CPS_VERSION`, `CPS_COMPAT_VERSION`, `CPS_VERSION_SCHEMA`, `CPS_LICENSE`, `CPS_DESCRIPTION`, `CPS_HOMEPAGE_URL`, `CPS_DEFAULT_TARGETS`, or `CPS_DEFAULT_CONFIGURATIONS`. Pass-through options not listed here, such as `CPS_DEFAULT_LICENSE`, may still be used when applicable. Plain `VERSION` may still be used for the wrapper's CMake config version.
...
- CMake may also inherit project metadata by default when the CPS package name matches the current project. Use `CPS_PROJECT` to select a different project, or `CPS_NO_PROJECT_METADATA` to disable project metadata inheritance.
- `CPS_COMPAT_VERSION` overrides compatibility metadata. Otherwise simple versions are derived from `COMPATIBILITY`: `AnyNewerVersion` -> `0.0.0`, `SameMajorVersion` -> `<major>.0.0`, `SameMinorVersion` -> `<major>.<minor>.0`, and `ExactVersion` omits `COMPAT_VERSION`. This derivation only applies when `CPS_VERSION_SCHEMA` is omitted or `simple`; for custom schemas, set `CPS_COMPAT_VERSION` explicitly.
...
- CMake's `EXPORT_FIND_PACKAGE_NAME` target property is experimental and gated by `CMAKE_EXPERIMENTAL_EXPORT_PACKAGE_DEPENDENCIES`; use that property only when the matching CMake experiment is enabled for your CMake version.
