...
# ~~~
# Collect ALL runtime dependencies for creating shared library containers.
#
# This function registers targets for complete runtime dependency collection
# using file(GET_RUNTIME_DEPENDENCIES) and installs ALL collected .so files
# (including system libraries) to a separate RuntimeDeps component.
#
# Purpose:
#   Extract ALL shared libraries into a common container that can be shared
#   across multiple application containers for memory efficiency.
#
...
#   - Collects EVERY dependency (no filtering of system libraries)
...
#   - Used with CPack to create separate tarballs for container import
#   - Enables lib-provider container pattern for shared memory optimization
#
# Examples:
...
function(_finalize_rdeps)
  ...
  # Convert targets to full paths for GET_RUNTIME_DEPENDENCIES We need to use generator expressions since target locations aren't known at configure time
  set(target_genexps "")
  ...
  # Create install rule that runs at install time Using a here-document for better readability of the install code
  install(
    CODE "
    ...
    # NO FILTERING - collect everything for maximum portability
    file(GET_RUNTIME_DEPENDENCIES
      EXECUTABLES \${resolved_executables}
      RESOLVED_DEPENDENCIES_VAR runtime_deps
      UNRESOLVED_DEPENDENCIES_VAR unresolved_deps
      # NO PRE_EXCLUDE_REGEXES - we want EVERY dependency
      # Filtering can be done at a later stage if needed
    )
    ...
    if(dep_count GREATER 0)
      ...
      message(STATUS \"No runtime dependencies found (likely static linking)\")
    endif()
  ...
  # Register the RuntimeDeps component for CPack integration This allows external tools to detect and work with the runtime dependencies
  ...
  project_log(VERBOSE "  Collecting: ALL dependencies (no filtering)")
  project_log(VERBOSE "  Symlink following: enabled")
  project_log(VERBOSE "  Purpose: Shared library container for memory optimization")
