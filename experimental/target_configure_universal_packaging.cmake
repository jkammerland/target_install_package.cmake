...
define_property(GLOBAL PROPERTY "_UNIVERSAL_PACKAGING_CONFIGURED"
...
function(configure_universal_packaging)
  set(options "")
  set(oneValueArgs NAME VERSION DESCRIPTION LICENSE MAINTAINER HOMEPAGE_URL SOURCE_URL SOURCE_DIR)
  set(multiValueArgs "")
  ...
  set_property(GLOBAL PROPERTY "_UNIVERSAL_PACKAGING_METADATA" 
    "NAME" "${ARG_NAME}"
    "VERSION" "${ARG_VERSION}"
    "DESCRIPTION" "${ARG_DESCRIPTION}"
    "LICENSE" "${ARG_LICENSE}"
    "MAINTAINER" "${ARG_MAINTAINER}"
    "HOMEPAGE_URL" "${ARG_HOMEPAGE_URL}"
    "SOURCE_URL" "${ARG_SOURCE_URL}"
...
function(configure_arch_packaging)
  ...
  # Store arch metadata
  set_property(GLOBAL PROPERTY "_UNIVERSAL_PACKAGING_ARCH_CONFIG"
    "MAKEDEPENDS" "${makedepends_str}"
    "DEPENDS" "${depends_str}"
    "OPTDEPENDS" "${optdepends_str}"
    "ARCH" "${ARG_ARCH}"
    "CUSTOM_BUILD" "${ARG_CUSTOM_BUILD}"
    "CUSTOM_PACKAGE" "${ARG_CUSTOM_PACKAGE}"
  )
...
function(configure_alpine_packaging)
  ...
  set_property(GLOBAL PROPERTY "_UNIVERSAL_PACKAGING_ALPINE_CONFIG"
    "MAKEDEPENDS" "${makedepends_str}"
    "DEPENDS" "${depends_str}"
    "CHECKDEPENDS" "${checkdepends_str}"
    "ARCH" "${ARG_ARCH}"
    "CUSTOM_BUILD" "${ARG_CUSTOM_BUILD}"
    "CUSTOM_PACKAGE" "${ARG_CUSTOM_PACKAGE}"
    "CUSTOM_PREPARE" "${ARG_CUSTOM_PREPARE}"
  )
...
function(configure_nix_packaging)
  ...
  # Store nix metadata
  set_property(GLOBAL PROPERTY "_UNIVERSAL_PACKAGING_NIX_CONFIG"
    "BUILD_INPUTS" "${build_inputs_str}"
    "PROPAGATED_BUILD_INPUTS" "${propagated_build_inputs_str}"
    "NATIVE_BUILD_INPUTS" "${native_build_inputs_str}"
    "FLAKE_ENABLED" "${ARG_FLAKE_ENABLED}"
    "CUSTOM_BUILD_PHASE" "${ARG_CUSTOM_BUILD_PHASE}"
    "CUSTOM_INSTALL_PHASE" "${ARG_CUSTOM_INSTALL_PHASE}"
    "CUSTOM_CONFIGURE_PHASE" "${ARG_CUSTOM_CONFIGURE_PHASE}"
  )
...
function(_create_pkgbuild_template output_dir components metadata arch_config is_binary)
  ...
  set(pkgbuild_content "# Maintainer: @MAINTAINER@
...
")
  ...
  if(NOT is_binary)
    set(pkgbuild_content "${pkgbuild_content}source=(\"@SOURCE_URL@\")
...
")
  ...
  # Add build function
  if(NOT is_binary)
    set(pkgbuild_content "${pkgbuild_content}build() {
    cd \"@SOURCE_DIR@\"
    ...
    if(ARCH_CUSTOM_BUILD)
      set(pkgbuild_content "${pkgbuild_content}
...
function(_create_apkbuild_template output_dir components metadata alpine_config is_binary)
  ...
  set(apkbuild_content "# Maintainer: @MAINTAINER@
# Generated by CMake target_configure_universal_packaging
...
")
  ...
  # Add source and checksums for source packages
  if(NOT is_binary)
    set(apkbuild_content "${apkbuild_content}source=\"@SOURCE_URL@\"
sha256sums=('SKIP')  # Replace with actual checksum
...
")
  ...
  if(ALPINE_CUSTOM_PREPARE)
    set(apkbuild_content "${apkbuild_content}prepare() {
    default_prepare
    cd \"@SOURCE_DIR@\"
...
")
  ...
  # Add build function
  if(NOT is_binary)
    set(apkbuild_content "${apkbuild_content}build() {
    cd \"@SOURCE_DIR@\"
    ...
    if(ALPINE_CUSTOM_BUILD)
      set(apkbuild_content "${apkbuild_content}
...
")
  ...
  # Add check function
  if(NOT is_binary)
    set(apkbuild_content "${apkbuild_content}check() {
    cd \"@SOURCE_DIR@\"
